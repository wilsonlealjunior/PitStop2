package pitstop.com.br.pitstop.pockdata;

import android.util.Log;

import pitstop.com.br.pitstop.util.NumberUtil;
...
//      return a;
  ...
  public static byte[] FramePack(byte typeFrame, byte[] srcBuff, int srcOffset, int srcLen)
  {
    ...
    if (typeFrame == FRAME_PHONE
      ...
      buffer[offset] = B_SOF;
      offset++;
        String t = StringUtil.toString(buffer);
//      buffer[offset] = B_SOF;
//      offset++;
//      buffer[offset] = B_SOF;
//      offset++;
      buffer[offset] = typeFrame;
        String t1 = StringUtil.toString(buffer);
      offset++;
      buffer[offset] = getDataID();
        String t2 = StringUtil.toString(buffer);
      offset++;
      intToAscii(srcLen, buffer, offset, BYTES_OF_DATA_LENGTH);
        String t3 = StringUtil.toString(buffer);
...
//      offset++;
      buffer[offset] = B_EOF;
        String t4 = StringUtil.toString(buffer);
      offset++;
      return buffer;
    }
...
//    }
  ...
  //  if (fontSet != FONT_DEFAULT)
   ...
   fontlinespace = new byte[]{B_ESC,(byte)0x33,linespace};
    ...
    String t = StringUtil.toString(buffer);
    String t2 = StringUtil.toString(formate);
    Log.i("formate value = ", StringUtil.toHexString(formate));
    String t3 = StringUtil.toString(concatByteArray(formate, buffer));
